...
//		Gen Starts Here!!!!!
		...
		while (true) { // each time the loop iterates: make room at previously determined (x,y), find a new (x,y) for next iteration
//				make room
...
//				we can now start pathing entrances through the previously generated room we encountered!
			} else if (map.getSpot(x,y) == RoomType.R1x1) { // we encounter a 1x1 room at (x,y)
				map.getRoom(x,y).makeFourWayConnectorRoom(); // turn this 1x1 room at (x,y) into a 4 way connector room
...
//			decide next room location
			...
			if (secondRandom == 1 && map.getRoom(check1[0], check1[1]) == null) { // set next room location
				...
				if (possibleChecks.isEmpty()) {
					// make current room 3 way?
					if (!currentRoom.isFourWayConnectorRoom()) {
						currentRoom.makeDeadEnd(previousMove);
					}
					break;
				}
